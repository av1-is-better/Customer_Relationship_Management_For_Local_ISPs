...
async def decode_access_token(token: str, AUTH0_CREDS:dict, ADMIN_EMAILS, db):
        ...
        try:
            ...
            if AUTH0_EMAIL_NAMESPACE in payload:
                ...
                if decrypted_email in ADMIN_EMAILS:
                    ...
                    # Verifying User Exist in User Database
                    check_user_query = text("""
                            SELECT email FROM users 
                            WHERE email = :email
                            """)
                    result = await db.execute(check_user_query,{"email":decrypted_email})
                    rows = result.fetchall()
                    if rows:
                        return decrypted_email
                    else:
                        # Verifying User Exist in Admin Database
                        check_admin_query = text("""
                            SELECT email FROM admins 
                            WHERE email = :email
                            """)
                        result = await db.execute(check_admin_query,{"email":decrypted_email})
                        rows = result.fetchall()
                        if rows:
                            return decrypted_email
                        else:
...
async def get_user_email(token: str = Depends(HTTP_BEARER),db: AsyncSession = Depends(get_db)):
    global AUTH0_CREDS
    global ADMIN_EMAILS
    return await decode_access_token(token.credentials,AUTH0_CREDS,ADMIN_EMAILS,db)